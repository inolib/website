...
export const UserInformationForm = component$(() => {
  ...
  const deleteModal$ = $<ModalToggleFunction>(() => {
    store.modal.attributes.hidden = !store.modal.attributes.hidden;
  });

  console.log(store.modal.attributes);

  return (
    <div>
      <form class="grid-rows-8 mx-[3rem] grid grid-cols-4 py-14 text-[#0B3168] md:grid-cols-9 md:grid-rows-4">
        <select
      ...
      <div {...store.modal.attributes} class="">
        <Modal />
      </div>
    </div>
  );
