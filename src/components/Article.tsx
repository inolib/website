...
export const Article = component$((props: ArticleProps) => {
  ...
  return (
    <article class="text-[#0B3168] md:mx-[8rem] md:py-8">
      <h2 class={`font-semibold mx-5 my-7 text-2xl md:m-0`}>{props.title}</h2>
      ...
      <div class={`flex flex-col ${stylesArticleGaucheFlex} md:pt-6`}>
        <span class="mx-5 md:mx-0">
          <span class={`flex ${articleTestimonyStyles} items-center`}>
            <h3 class={`font-bold mb-2 text-xl md:mb-0 ${stylesTestimony}`}>{props.testimony}</h3>
          ...
          <strong class="flex justify-center -mt-10 mb-3  text-justify md:mb-0 md:mt-16 md:text-xl">
        ...
        <div class={`flex flex-col ${stylesImgArticle}`}>
          ...
          <blockquote class={`my-6 max-w-content text-center ${imgQuoteSize}`}>{props.imageQuote}</blockquote>
          <p class={`text-center max-w-content ${styles} ${imgQuoteSize} md:mt-6`}>{props.imageText}</p>
        </div>
      </div>
    </article>
  );
