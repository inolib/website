...
import { fetchOperational } from "~/app/utils/fetchPolicy";
...
export const OperationalSupport = async ({ _color }: OperationalSupportProps) => {
  const { sectionClassName } = variants({ _color });

  const { titre, image, contenu } = await fetchOperational();
  ...
  return (
    <Section className={sectionClassName()}>
      ...
      <Verbose>
        ...
        <VerboseIllustration>
          <Image alt="" className="h-96" height={500} src={image} width={400} />
        </VerboseIllustration>
      </Verbose>
    </Section>
  );
